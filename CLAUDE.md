...
React PWA (Vite + Tailwind + Zustand) · FastAPI (Python) · Supabase (Postgres + Auth + RLS) · Anthropic API (Messages + `tool_use` + `web_search_20250305` server tool for card-multiplier lookup, with `allowed_domains` allowlist) · Google Gemini 3.1 Flash-Lite (`gemini-3.1-flash-lite-preview`, currently in preview) · Resend · PostHog · Sentry · Railway (backend) + Vercel (frontend, static + CDN; cross-origin via CORS with Bearer-token auth — DESIGN.md §5.3, §9.3) · Postgres `pg_cron`.

Stripe is documented in the forward plan only — not a v1 dependency. Perplexity Sonar was the original card-lookup vendor in DESIGN.md v3.1; replaced by Claude `web_search` during Day 14 planning (DESIGN.md §0 and §6.1) — one fewer key, SDK, and sub-processor, with an enforced source allowlist and richer citations.
...
1. **RLS is enforced via the user's JWT, not the service role.** FastAPI receives `Authorization: Bearer <user_jwt>` and instantiates a per-request Supabase client passing that JWT. Postgres enforces `auth.uid() = user_id` automatically. In v1, the `SUPABASE_SERVICE_ROLE_KEY` is reserved for **callers with no user JWT in scope**: (1) the `pg_cron` daily auto-logger (DB function, no app context); (2) Supabase CLI migrations; (3) the weekly digest cron job (`app/cron/digest.py`, DESIGN.md §6.4) and any future scheduled email job that iterates users — the cron has no user JWT for the recipient by definition; (4) the Resend bounce/complaint webhook (`app/routes/webhooks_resend.py`, DESIGN.md §6.4) — the request is from Resend, not a logged-in user; (5) the one-click unsubscribe route (`app/routes/unsubscribe.py`, DESIGN.md §6.4, RFC 8058) — the request is authenticated by the HMAC token in the email link, not a user JWT (the recipient may have no session on the device opening the link). Application **request handlers triggered by a user** never use the service role. If you find yourself reaching for it in a request handler, stop — you're about to bypass RLS. The structural test (`tests/contracts/test_no_service_role_leak.py`) excludes `app/cron/` and `app/scripts/` by directory and lists the webhook and unsubscribe files in a per-file allowlist with rationale comments — widening either requires the same rationale-comment discipline as `ALLOWED_DIRECT_WRITE_TOOLS`. **Forward-plan addition (only if the §17 scaling plan activates):** a Stripe webhook handler would become a sanctioned sixth caller, because no user JWT is in scope for a Stripe-initiated request — it resolves `user_id` via `stripe_customer_id` on `users_meta`. Do not add this path until the scaling decision is made.
...
3. **MCP server is read-only and authenticates via OAuth 2.1.** Authorization is delegated to Supabase Auth's OAuth 2.1 Server — Tameru's MCP server is an OAuth 2.1 *Resource Server*, not its own credential store, so there is no `mcp_tokens` table (DESIGN.md §7.9, §8.6). OAuth is required because Claude.ai's web connector UI accepts no static bearer token or custom header. No `add_transaction` over MCP in v1 — a leaked or over-scoped credential reads data; it cannot mutate it. Write tools, if ever added post-launch, require explicit per-grant scopes and user approval. (The original v3 design — per-user 32-byte bearer tokens stored as `sha256` — was superseded during Day 23 planning.)
...
5. **Single active device per user.** `users_meta.active_device_id` is claimed only by acts of user intent — explicit sign-in (the Supabase `SIGNED_IN` event), `/auth/bootstrap` (server-side, same transaction), or the displaced modal's "use here" button. Never as a side effect of a token refresh, `INITIAL_SESSION`, or page reload: the original unconditional claim let a displaced device silently re-take the slot on its next 300s-TTL refresh, ping-ponging `active_device_id` between two open devices forever (2026-06 audit P2-2). The previous device is *displaced, not revoked*: its Supabase session stays valid, but every device-gated API call 401s with `DEVICE_DISPLACED` until the user explicitly re-claims or signs in again — the gate is API-level, which is sufficient because the invariant's purpose is one *writer* at a time (reads are RLS-scoped to the same user either way). There is no offline multi-device conflict resolution because there can be no concurrent writer.
...
| Card multiplier lookup | `claude-haiku-4-5` + `web_search_20250305` server tool | One call per card add (1–2 internal searches, `max_uses=3`). `allowed_domains` allowlist: NerdWallet, The Points Guy, US Credit Card Guide, Doctor of Credit, inferred issuer domain. Citations from `web_search_result_location.url` stored as `cards.source_urls`. Replaces the original Perplexity Sonar plan (DESIGN.md §0 and §6.1). |
