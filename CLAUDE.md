...
8. **Chat is the only user-initiated write surface in v1.** Transactions, cards, and subscriptions are all created by the user typing or speaking in chat, which routes to the Claude Haiku agent. Claude extracts fields via `tool_use` args — **there is no separate Gemini NL-parse call in the chat path**. Writes use a propose-then-confirm pattern: the tool returns a proposal payload, the UI renders it as a preview card (UX frame 15), and a `POST /<resource>/confirm` endpoint writes the row only after the user taps "looks right." There is no separate `+`-button entry form in v1; do not add one. Edits and deletes happen in the edit sheet (UX frame 11b), reached from the per-category transaction list (UX frame 11a) or from an in-chat candidate list rendered when Claude's `get_transactions(...)` returns multiple matches. Chat does not directly mutate transactions — its write role is the proposal step, and its edit role is retrieval only. CSV import and receipt photo (deferred) remain Gemini-parsed — those are bulk/async, not user-typed chat text.
