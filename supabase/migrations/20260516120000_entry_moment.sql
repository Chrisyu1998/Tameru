-- entry_moment — Day 13 (DESIGN.md §6.2)
-- Rate-limit ledger + single-query candidate-signals RPC for the entry-moment
-- insight rule engine. The Python side calls `entry_moment_signals(txn_id)`
-- once per confirm to gather every signal the four rules need; without this
-- consolidation the service would stack 4 sequential Supabase round-trips
-- and blow the latency budget (Day 13 prompt).
--
-- Audit-immutable posture matches ai_call_log (CLAUDE.md invariant 14):
-- entry_moment_fires has SELECT and INSERT policies only. A compromised JWT
-- can read its own rate-limit history but cannot delete a fire row to bypass
-- a suppression window.

CREATE TABLE entry_moment_fires (
    id          UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id     UUID        NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    rule_id     text        NOT NULL,
    category    text        NOT NULL,
    fired_at    timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT entry_moment_fires_rule_check
        CHECK (rule_id IN (
            'single_tx_notable',
            'weekly_frequency',
            'cumulative_delta',
...
CREATE INDEX entry_moment_fires_user_fired_idx
    ON entry_moment_fires (user_id, fired_at DESC);
CREATE INDEX entry_moment_fires_user_rule_idx
    ON entry_moment_fires (user_id, rule_id, fired_at DESC);

ALTER TABLE entry_moment_fires ENABLE ROW LEVEL SECURITY;
ALTER TABLE entry_moment_fires FORCE  ROW LEVEL SECURITY;

CREATE POLICY entry_moment_fires_select ON entry_moment_fires
    FOR SELECT
    USING (user_id = auth.uid());

CREATE POLICY entry_moment_fires_insert ON entry_moment_fires
    FOR INSERT
    WITH CHECK (user_id = auth.uid());


-- entry_moment_signals(p_transaction_id)
-- Returns one row containing every signal the Python rule engine evaluates.
-- SECURITY INVOKER so per-user RLS on transactions, cards, and
-- entry_moment_fires scopes the reads to the caller's JWT — a leaked
-- function name cannot read another user's data.
--
-- "Week" here means "the 7 days ending on the transaction's date,
-- inclusive" — a rolling window, not ISO week, because the rule sentences
-- read more naturally that way ("4th dining transaction this week").
-- "Month" is the calendar month of the transaction's date, since
-- transactions.date is already stored in the user's home timezone
-- (CLAUDE.md invariant 13 / DESIGN.md §8.7).
...
BEGIN
    ...
    -- RLS denial or missing row → return no rows; caller treats as "no insight".
