# Day 16 — All typed tools (reads + propose-then-confirm writes) + middleware
...
**No write happens inside the tool.** Each of these returns a proposal payload; the React client renders the preview card; a separate `POST /<resource>/confirm` endpoint (Day 5 for transactions, Day 11 for cards, Day 14 for subscriptions) writes the row after the user taps "looks right."
...
- `propose_card({network, last4, program, alias?})` → `CardProposal`. Tool impl calls Perplexity (Day 11 `lookup_card`) to populate multipliers and source_urls, returns the proposal. **Does not INSERT.**
- `propose_subscription({name, amount, frequency, start_date, category?, card_id?})` → `SubscriptionProposal`. Tool impl computes `next_billing_date` from `start_date + frequency`, returns the proposal. **Does not INSERT.**
...
- On cap exceeded: surface `{"code": "DAILY_CAP_EXCEEDED", "message": "You've used your daily AI quota — resets at midnight UTC."}` for Day 18's UI.
