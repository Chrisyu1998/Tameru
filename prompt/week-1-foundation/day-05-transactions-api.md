...
- **`POST /transactions/confirm`** — body: `TransactionProposal` payload (merchant, amount, date, card_id, category, notes?, source="manual"|"nlp"|"auto_logged"|"csv_import", gemini_suggestion?). Validates against the closed category enum. Inserts the row. Returns `{transaction: Transaction, insight: str | null}` where `insight` comes from Day 13's deterministic entry-moment insight function. **Stub note:** Day 13 now lands *after* Day 5 in the reordered plan. Ship Day 5 with `insight: null` hardcoded in the response — the `insight` field exists in the schema from day one so the frontend can treat it uniformly. Day 13 wires `entry_moment_insight()` into this endpoint when it lands.
  - `source` defaults to `"nlp"` when the chat path calls it; other writers (CSV import Day 20, subscription auto-logger Day 19) pass their own source.
...
- `GET /transactions` — query params: `card_id?, category?, merchant_contains?, date_from?, date_to?, amount_min?, amount_max?, limit?=50, offset?=0`. Returns `{items, total, has_more}`. RLS-scoped (uses `supabase_for_user`). Powers both the list UX (frame 11a) and the agent's `get_transactions` tool (Day 9).
...
- Don't add subscription auto-logging here — Day 19 owns that.
