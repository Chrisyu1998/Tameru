...
- Card recommender, proactive insights, recurring detection, retroactive rewards gap analysis. (Receipt photo → transaction via Gemini Vision was pulled into v1 — see §6.2 "Receipt photo entry" and the model table in §5.4.)
...
| Receipt photo → transaction (Gemini Vision) | 1 | P1 (shipped) |
...
> **Region-aware (Tier 3, §6.6).** US cards use the category-multiplier lookup described below against US sources (`UR/MR/TYP/Bilt`, NerdWallet/TPG/etc.). JP/TW cards (routed by the card's `region`) use a base-rate-only lookup against local sources — a single `base_reward_rate` + free-text `rewards_currency` label, no category multipliers (see §6.6 for why). The long tail of non-US issuers and any non-US category bonuses remain deferred — see §6.6 and `TODO.md`.
...
**Receipt photo entry (v1):** a second entry point onto the *same* propose-then-confirm flow. A camera button in the chat composer (and the desktop composer) lets the user photograph — or pick — a receipt. The image is downscaled + JPEG-re-encoded on-device (`lib/image.ts`: a max-1600px long edge, EXIF orientation baked, which also sidesteps iOS HEIC since Safari decodes HEIC to a canvas and exports JPEG) and posted to **`POST /receipts/parse`**. That route runs exactly one Gemini Vision call (`parse_receipt`, `task_type='receipt_parse'`, logged under the user's JWT per invariant 14), extracts merchant / amount / date, and returns a standard `TransactionProposal` with `source='receipt_photo'` — built by the shared `build_transaction_proposal()` helper, so date-defaulting, `categorize()`, and the `client_request_id` mint are identical to the chat path. The proposal renders as the same parse card and commits via the same `POST /transactions/confirm`. Two load-bearing properties: (1) the image **never enters the Haiku agent loop** — it goes straight to Gemini Vision, keeping invariant 8 intact (the HTTP confirm is the only commit; there is no `tool_use` write); (2) the image is **never stored** — no Supabase Storage bucket; the bytes are a request-local value discarded after the Gemini call. Category comes from the normal `categorize()` path (not the vision call) so the merchant-correction learning loop is unchanged. One transaction per receipt (the grand total); no itemization, no FX (invariant 13) — the amount is editable on the parse card. No DB migration was needed beyond re-adding `receipt_parse` to the `ai_call_log` task_type CHECK (it had been dropped when receipt was out of scope); `transactions.source` already permitted `receipt_photo`.
...
2. **One-click List-Unsubscribe** per RFC 8058 — every email carries `List-Unsubscribe: <https-url>, <mailto>` + `List-Unsubscribe-Post: List-Unsubscribe=One-Click`. The URL is HMAC-tokenized `…/unsubscribe?user=…&kind=digest&token=…` (signed with `DIGEST_UNSUBSCRIBE_SECRET`, no expiry — a year-old link still works); the flip happens via service role, no login required. **The mutation lives exclusively on the POST** (the RFC 8058 one-click verb that Gmail/Yahoo send): `GET /unsubscribe` — what a human clicking the visible body link sends — verifies the token and 302s to the PWA's `/unsubscribe` confirm page, whose button POSTs the token back. A mutating GET silently unsubscribed users whose corporate mail scanners (Outlook SafeLinks, Mimecast) prefetch every link in the email body (2026-06 audit P3-10); scanners don't POST RFC 8058 endpoints, so a POST is intent. The confirm page is a PWA route (not backend-served HTML — the §14.5/Day-27 CSP posture keeps FastAPI JSON-only) and needs no session: the token in the query string is the authorization. Also a **visible** Unsubscribe link in the email body (defense in depth — header alone is insufficient; that's what users actually find). Gmail's ≥5K/day threshold for *required* one-click doesn't apply at v1 scale, but ship it anyway — inbox placement benefits and retrofit-after-reputation-damage is much harder than ship-it-now.
...
**Tier 2b — UI chrome translation (shipped).** react-i18next (`i18next` + `react-i18next`) is wired with **the store's `uiLanguage` as the single source of language truth** — `main.tsx` calls `i18n.changeLanguage(...)` from the same effect that sets `<html lang>`, so there is no second, competing language state and no `i18next-browser-languagedetector`. Resources are static JSON (`src/locales/{en,ja,zh-TW}.json`, one `translation` namespace nested by surface — `nav.*`, `settings.*`, `chat.*`, …); `en` is the source-of-truth copy captured verbatim from the original JSX (so English rendering is byte-identical), `fallbackLng: 'en'` covers any gap. ~600 keys per language cover every page and component's hardcoded English (buttons, headings, placeholders, aria-labels, empty-states, errors, onboarding/tour/privacy prose). The remaining *interactive/edit* category sites from 2a — the edit-sheet pickers (`EditTransactionSheet`, `EditSubscriptionSheet`) and the chat parse cards — now localize their displayed category via `useCategoryLabel()` while the selected/stored value stays the English enum. ja/zh-TW are initial drafts for native-speaker (family) review; `en` is final. Tests initialize i18n in the Vitest setup so component assertions resolve real English copy. The brand wordmark "tameru" is intentionally never translated.
...
**Tier 3 — international credit cards (minimal slice shipped 2026-06-02; breadth deferred).** See §6.1 and `TODO.md`. Card reward lookup is now region-aware: US cards get the category-multiplier lookup against US sources; JP/TW cards get a base-rate-only lookup against local sources. JP/TW users add cards through the normal flow with the top ~6 JP and ~6 TW issuers enumerated. Still deferred: the long tail of JP/TW issuers, promos / category multipliers outside the US, and per-card branding + localized AF-recognition for JP/TW.

**Scope decision (resolved 2026-06-02, recorded in `TODO.md`):** (1) **Multi-region cards in one wallet are supported** — `region` is a per-card column (`cards.region`, default 'US', CHECK US/JP/TW), not a per-user property, so a wallet can hold a US card and a TW card at once (the "moved back to Taiwan, still carries US cards" case); lookup routes per card. The region is recomputed server-side at `/cards/confirm` from the resolved issuer (a known issuer pins it, forge-resistant; falling back to an explicit pick, then to `home_currency`, for the `other`/unknown issuer). Because **chat is the only reachable card-add surface in v1** (invariant 8 — the onboarding `AddCardStep` form, which carries a region selector, is bypassed on the natural flow), the per-card region signal comes from the **agent**: the `propose_card` tool takes an optional `region` arg that Claude infers from the issuer/card name (Chase → US, Rakuten → JP, Cathay → TW), routing the lookup to the right sources; it falls back to the user's `home_currency` region when Claude can't tell. This is what lets a Japan-based user add a US card by chat (`region="US"` → US lookup → issuer pins `US`). The form's selector is latent (correct if that surface is ever revived). (2) **Multi-currency spend is not** — invariant 13 stands (one immutable `home_currency`, no per-transaction currency, no FX); a foreign-denominated card's purchases are hand-converted into home currency, and a true dual-currency ledger is permanently out of scope (§3.2). (3) **Permanent relocation → new account** — a user who moves countries and needs a different home currency deletes + re-signs-up (the invariant-13 escape hatch); there is no home-currency migration path. (4) **Outside the US, the lookup captures the base earn rate only — no category multipliers, no promos** — the one-shot, no-refresh lookup pipeline can't stably represent partner-economy or user-selected bonus categories, so JP/TW cards store a nullable `base_reward_rate` + free-text `rewards_currency` label. Annual fee resolves in the user's `home_currency` (prompt wording only, no FX; fail closed to null on a currency mismatch). **Issuer model (as built):** the `network` and `issuer` CHECKs + Pydantic `Literal`s were *widened* (network gains `jcb`/`diners`; issuer gains the JP/TW banks), keeping the normalized key the `(user_id, issuer, last_four)` unique index relies on; a `card_issuers` reference table (`key`/`region`/`display_name`/`domain`, migration `20260602120100`) holds the region/domain metadata, mirrored in `app/integrations/card_regions.py` and `frontend/src/lib/cardsApi.ts`. The `program` enum is unchanged — JP/TW cards store `'Other'` and surface rewards via `rewards_currency`. Migrations + the minimal-done slice are detailed in `TODO.md`.
...
**Nav placement.** A dedicated **Credits page** at `/cards/:cardId/credits`, reached from a "credits" chip on the card row (`CardTile`, the AF-chip sibling — there is no separate card-detail page in v1), not a top-level BottomNav tab (avoids mobile nav bloat). An optional all-cards roll-up entry can live on the cards page / More menu.
...
**Status: Phase 1 and Phase 2 shipped 2026-07-05.** Phase 2 (migrations `20260705120700`–`20260705121000`) added: the **ledger bridge** — a `credit_suggestion` field on `POST /transactions/confirm` (a *separate* field from the entry-moment `insight`, so the two never suppress each other) committed by `POST /card-credits/{id}/apply` through the **idempotent** `card_credit_apply_usage` RPC. A `(card_credit_id, transaction_id)` ledger (`card_credit_applications`, §8.19) dedupes replays so a lost-response retry / double-tap can't double-count the same purchase (Codex 2026-07-05); the increment is clamped `[0, allowance]`, guarded on same-card + `date >= current_period_start`. Also: the **weekly-digest** "use it or lose it" + "value captured" lines (14-day window, `compose_digest`); **`card_credit_history`** period snapshots (§8.18) + `GET /card-credits/{id}/history`; and the read-only **`get_card_credits`** tool, exposed on both the chat agent (`chat_v16`) and the MCP server. Anniversary anchoring, an entry-moment credit nudge, and auto term-refresh remain deferred (`TODO.md`).
...
- `get_transactions({ category?, card_ref?, merchant_contains?, date_from?, date_to?, amount_min?, amount_max?, limit? }) → Transaction[]`
- `calculate_total({ category?, card_ref?, date_from?, date_to? }) → { total, count }`
- `get_subscriptions({ status? }) → Subscription[]` — each row carries a resolved `card_ref` (not the raw `card_id` UUID), so "which card pays for X?" never requires the model to cross-match UUIDs.
...
- `get_cards() → Card[]` — each card carries a short `ref` handle (`{issuer}-{last_four}`, e.g. `amex-1001`) for the propose tools' and read filters' `card_ref` args; card rows are UUID-free (no `id`) since chat_v14 — nothing in the agent path consumes one.
...
**Card references use a short handle, not the UUID.** The agent identifies a card via `get_cards` and passes that card's `ref` (`{issuer}-{last_four}`) — never a UUID — to `propose_transaction` / `propose_subscription`'s `card_ref` AND to `calculate_total` / `get_transactions`' `card_ref` filter (the read tools joined in chat_v14 — the audit found they still took a raw `card_id`, the exact transcription path chat_v10 closed for proposals). The tool resolves the handle to the UUID server-side under RLS. Rationale: the §7.10 chat-extraction eval caught Claude dropping a hex digit while copying a 36-char UUID between `get_cards` and `propose_subscription`, silently losing the card attribution. A short, meaningful handle is reliable to copy, and a slip fails closed rather than mis-resolving — with direction-appropriate semantics: a proposal ref that doesn't resolve degrades to a cardless proposal the user corrects on the parse card, while a *read-filter* ref that doesn't resolve raises (is_error tool_result) because silently matching nothing would return `$0` presented as exact. The tool implementations still accept a `card_id` UUID for direct (non-agent) callers and tests, but the agent-facing schemas expose only `card_ref`, and tool results carry no UUIDs at all.
...
| Receipt photo parsing | Gemini (env-resolved, multimodal) | One Vision call per receipt (`parse_receipt`, `task_type='receipt_parse'`). The camera button lives in the chat composer, but the image goes to `POST /receipts/parse` (Gemini) — NOT through the Haiku agent loop — and returns a standard `TransactionProposal` committed via the existing propose-then-confirm. See §6.2 "Receipt photo entry". |
| CSV header + batch parse | Gemini (env-resolved) | Unstructured → structured, batched. Bulk/async path, not chat. |
| Chat-based transaction extraction | Claude Haiku 4.5 | In v1, chat is the only user-initiated write surface (CLAUDE.md invariant 8). Claude reads the user's message and fills `propose_transaction(...)` args directly via `tool_use`. There is no separate Gemini NL-parse call in the chat path. Gemini remains the parser for the CSV and receipt paths above because those are Gemini-parsed with no agent loop involved (CSV is bulk/async; receipt is a single Vision call whose entry point happens to be the chat composer — the image never enters the Haiku loop, and it returns a `TransactionProposal` committed via propose-then-confirm). |
...
**Layer 2 — cross-session profile.** A background Claude Haiku call reads a conversation and extracts atomic facts into `user_memory`. Each fact is one row (not a blob), enabling targeted deletion and time-decay pruning.

**Triggering (piggyback on `POST /chat/turn`, no timer/daemon).** Two probes run per turn, each scheduling `distill_session` as a `BackgroundTask` (JWT-in-request-scope; runs after the SSE stream closes):

- **Current-conversation probe** (`find_conversation_to_distill`, 2026-07-03) — distills the conversation the user is *actively in* once it reaches `MIN_CONVERSATION_MESSAGES` (4) committed messages, and re-distills it every `REDISTILL_DELTA` (4) further messages. This is the primary trigger: it does not require a return-visit, so a single-sitting session still builds memory. Re-distillation is safe because organic re-mentions dedup via `user_memory`'s unique index; `conversation_distillation_state.message_count` records how far a conversation was distilled through, and `distill_session` skips a re-run until the live count exceeds it by the delta. The state row is therefore *upserted*, superseding the original append-only "distill once, ever" posture.
- **Idle backstop** (`find_idle_undistilled_conversation`) — catches a *prior* conversation whose last message is >10 minutes old and either was never distilled or has **grown by `REDISTILL_DELTA` since its last distillation** (same count-delta predicate as the current-conversation probe, not a bare `NOT EXISTS`). This recovers the tail of a conversation that grew after its first distillation and was then abandoned before the current-conversation probe caught the growth. Excludes the current conversation by construction. Both probes take `min`/`delta` as arguments from `MIN_CONVERSATION_MESSAGES`/`REDISTILL_DELTA`, so the thresholds have one source of truth. The state row is written by a **monotonic** upsert (`upsert_conversation_distillation_state`, `GREATEST` on conflict) so an out-of-order straggler task cannot regress `message_count`.

**Prompt (`memory_distill_v3`).** Deliberately *generous* — most real conversations contain something worth remembering, so it errs toward capturing a low-scored fact rather than returning `[]` (a conservative extraction prompt is a known low-recall failure mode). It reframes ledger-heavy chat as a source of *patterns and intent* ("logged a $47 Trader Joe's run" → *User grocery-shops at Trader Joe's*), while still excluding live inventory and one-off specifics (card ownership, active subscriptions, individual transaction amounts) that the live tools own. Few-shot examples anchor both a populated output and a correct empty one.
...
- A missing *date* is not a hole. When the user states a merchant and amount but no time ("$7 at Peet's"), the agent **omits** `date` and the tool fills the user's local *today* server-side (`user_local_today`, resolved under RLS in `users_meta.timezone`, UTC fallback) — it does **not** ask "when was this?". The parse card (UX frame 15) is an editable correction surface, so a wrong default costs one tap, whereas a clarifying-question round-trip on every dateless entry is friction on the app's most common action. A missing *amount* still warrants a question — a proposal needs a real number. (Resolved Day 22, prompt `chat_v9`, after the §7.10 chat-extraction eval surfaced the agent over-asking. The blanket "don't fabricate dates" rule is scoped to retrieval windows, not entry. `chat_v15` moved the default *off the model*: the no-date path no longer routes the date through the LLM at all — the agent omits `date` rather than copying the injected "Today is …" anchor into the tool call, so a mis-read anchor or a UTC/local off-by-one can no longer mis-date the row. The model still computes explicit/relative dates like "yesterday" from the anchor.)
...
**Weekly drift run.** In addition to the per-PR `eval-gate`, a scheduled GitHub Actions workflow (`.github/workflows/weekly-eval.yml`) runs the full suite **weekly** against the env-resolved production models, with the corpus, prompts, thresholds, and judge byte-for-byte unchanged. It catches the one failure class the per-PR gate is structurally blind to: **model drift under unchanged code** — the chat and Gemini models are resolved from env (the model table above), so a provider can change behavior with zero commits. The constant corpus is the instrument; a score drop is attributable to the model. The judge runs on every weekly run (no day-of-week gate). On a gate breach the workflow files or comments on a deduped `eval-regression` GitHub issue carrying the breached gates and failing rows (written `claude-plan`-ready for the Day 31 dev loop; the label is applied by hand — human triage stays between detection and planning). Results are never committed: the `weekly-eval-run` artifact and the issue stream are the record. A transient provider failure that breaches a gate for one run self-heals on the next weekly run; an infrastructure failure (the local stack failing to boot) files no issue. This is a standalone workflow, not `schedule:` on `ci.yml` (memory.md 2026-05-28: ci.yml's post-deploy `needs:`/`if:` chain would skip exactly the jobs a scheduled run wants), mirroring `prod-health.yml`.
...
**RLS enforces card ownership on writes (migration `20260705120600`).** The `subscriptions_owner` policy's `WITH CHECK` requires `card_id IS NULL OR EXISTS (SELECT 1 FROM cards c WHERE c.id = subscriptions.card_id AND c.user_id = auth.uid())` — not just `user_id = auth.uid()`. The original owner-only policy checked only the row's own ownership, so a direct PostgREST caller could attach a subscription to another user's `card_id` (the FK only verifies the card exists *somewhere*); the route's `_assert_card_owned` was the sole defense. Per invariant 1 the database is the tenant boundary, so the check now lives in the policy too. `NULL` is allowed for cardless ACH bills. The SECURITY DEFINER writers (AF dual-write, `update_card_af`, `soft_delete_card`, the auto-logger) bypass RLS and are unaffected; the only RLS-subject write path (`/subscriptions/confirm` + PATCH card reassignment) already resolves `card_ref` to an owned card. This closed the same gap Codex flagged on the new `card_credits` table (§8.17).
...
**Confirm seeds the period bounds (as built).** `POST /card-credits/confirm` writes through the `card_credits_confirm(p_rows jsonb)` SECURITY INVOKER upsert (migration `20260705120200`), the plpgsql function the expression/partial `card_credits_active_name_uniq` index forces (it targets that index with `ON CONFLICT DO NOTHING`). It seeds `used_amount = 0` and `current_period_start` / `next_reset_date` from `credit_period_bounds(cadence, on_date)` (migration `20260705120100`) — the single calendar-math source of truth shared with `reset_card_credits()`, computed on the user's local today (`users_meta.timezone`, else UTC). The function also drops any row whose `card_id` is not the caller's active card. A `PATCH` that changes `cadence` recomputes both period dates via the same helper.

**Reset (`pg_cron reset_card_credits()`).** Daily sweep, same family as `autolog_subscriptions` (§6.5): for each `status = 'active'` credit with `next_reset_date <= today`, snapshot the closing period into `card_credit_history` (§8.18, migration `20260705120800`), then set `used_amount = 0`, `current_period_start` = the new period start, `next_reset_date` = the next calendar boundary for the cadence. Naturally idempotent (only fires past the boundary, then advances past today) and forward-only (never backfills). Advisory-locked against concurrent runs. Boundaries computed in the user's `timezone` (§8.7) when set, else UTC. System caller (no user JWT) → service role, the same sanctioned standing as the auto-logger (invariants 1 and 4).

**RLS shape:** `ENABLE`/`FORCE ROW LEVEL SECURITY`; owner `SELECT`/`INSERT`/`UPDATE`/`DELETE` policies scoped to `user_id = auth.uid()`. The `INSERT`/`UPDATE` `WITH CHECK` clauses *additionally* require `card_id` to reference one of the caller's own cards (`EXISTS (SELECT 1 FROM cards c WHERE c.id = card_credits.card_id AND c.user_id = auth.uid())`) — without it a direct PostgREST caller could bypass the confirm route to attach a credit to another user's `card_id` (a cross-tenant FK attachment + card-id existence oracle; Codex 2026-07-05). Per invariant 1 the database, not app code, is the tenant boundary, so the ownership check lives in the policy even though the route/RPC already filter owned cards. All app writes (confirm, usage edits, archive) run under the user's JWT; only the `pg_cron` reset uses the service role. (The same card-ownership `WITH CHECK` was retro-applied to `subscriptions`, which carried the identical `card_id`-FK gap, in migration `20260705120600` — see §8.3.)

### 8.18 `card_credit_history` (Phase 2 — §6.7)

Period snapshots so the Credits page can show "last quarter you used $60 / $75." **Built (Phase 2, migration `20260705120700`).** Written by the `reset_card_credits()` sweep at each period rollover (migration `20260705120800` — the closing period's `name` + `used_amount` + `amount` + the period bounds, captured *before* `used_amount` is zeroed), `ON CONFLICT (card_credit_id, period_start) DO NOTHING` for idempotency. `name` / `amount` are snapshotted (not read live off `card_credits`) so a later rename or amount edit doesn't rewrite what the closed period actually was. `(card_credit_id, period_start)` unique. RLS is **SELECT-only** for the owner — the sweep (SECURITY DEFINER) is the sole writer, so a user JWT cannot forge or scrub history (same posture as `ai_call_log`, invariant 14). Surfaced via `GET /card-credits/{id}/history` and the Credits page "last period" line.

### 8.19 `card_credit_applications` (Phase 2 — §6.7)

Idempotency ledger for the ledger bridge. **Built (Phase 2, migration `20260705121000`; Codex 2026-07-05).** One row per `(card_credit_id, transaction_id)` recording that a transaction was counted toward a credit; `UNIQUE (card_credit_id, transaction_id)` is the idempotency key. The `card_credit_apply_usage` RPC INSERTs here `ON CONFLICT DO NOTHING` and increments `used_amount` only when a *new* row lands — so a `POST /card-credits/{id}/apply` retry (lost response), double-tap, or offline replay of the same transaction is a no-op instead of double-counting the purchase. RLS is owner **SELECT + INSERT** only (no UPDATE/DELETE — immutable audit; cascades handle teardown), and the INSERT `WITH CHECK` independently verifies both the `card_credit_id` and `transaction_id` FKs belong to the caller (the cross-tenant FK guard, §8.17 / memory 2026-07-05). Not otherwise surfaced through the API in v1 (internal correctness ledger; a future "which purchases counted" view could read it).

**Reset/apply race (why the period guard is in the UPDATE).** The increment carries the period guard (`t.date >= cc.current_period_start`) inside its *own* UPDATE WHERE, joining the transaction — not just in the preceding idempotency INSERT. Under READ COMMITTED a plpgsql function shares its calling statement's snapshot, so if `reset_card_credits()` advances the period between the apply's snapshot and its write, only the *updating* statement's EvalPlanQual re-read sees the new period; a plain `SELECT … FOR UPDATE` guard would still read the stale (pre-reset) period and count an old-period spend into the fresh one. With the guard in the UPDATE, EvalPlanQual re-checks it under the row lock and the old-period spend is correctly rejected (`used_amount` unchanged). In that (astronomically rare) race the application row is recorded without a matching increment — benign: the spend is genuinely old-period, and a retry stays idempotent.
...
1. The `pg_cron` daily sweeps (run as DB functions, no application context): the subscription auto-logger and the credit-tracking `reset_card_credits()` reset (§6.7).
...
5. The one-click unsubscribe route (`app/routes/unsubscribe.py`, §6.4, RFC 8058) — the request is authenticated by the HMAC token embedded in the email link, not a user JWT; the recipient may not even have an active session on the device that opens the link.
...
> When you chat with Tameru, your message goes to Anthropic — that covers the chat agent (adding transactions, answering questions) and the `web_search` card-multiplier lookup (public card name + last 4 only, never transaction data). Anthropic's default 30-day trust & safety retention drops to zero under ZDR (requested for the Tameru org); not used for training under any tier. Google Gemini receives the merchant name and amount of each transaction to pick a category (the `propose_transaction` Gemini fallback when Claude doesn't supply one), and parses CSV imports. When you scan a receipt, the **receipt image itself** goes to Gemini Vision (it can show line items, a store address, or a partial card number); it is used only to read the merchant, total, and date, and the image is not stored — Tameru keeps only the transaction you confirm. Gemini is on its paid tier, which does not use API data for training; the free tier is forbidden.
...
**Webhook handler responsibility:** the webhook endpoint runs with the Supabase service role (invariant: no user JWT is in scope for a Stripe-initiated request — this would be the *sixth* sanctioned service-role caller (after pg_cron, CLI migrations, the digest cron, the Resend webhook, and the unsubscribe route); CLAUDE.md invariant #1 must be updated to admit it). It looks up the user by `stripe_customer_id`, updates `plan` / `stripe_subscription_status` / `stripe_current_period_end` on `users_meta`, and records the event ID in `stripe_events`. All writes are idempotent against the `stripe_events.event_id` primary key.
...
- [ ] **Service-role key audit.** Grep the codebase. Confirm no request handler imports `SUPABASE_SERVICE_ROLE_KEY`. The CI lint from §9.1 enforces this; confirm it's configured and fails the build on violation. The Stripe webhook handler becomes a sanctioned sixth caller (§17.2) — update the lint's allowlist accordingly.
