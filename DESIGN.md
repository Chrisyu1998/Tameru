...
**Tier 2 — translated UI (planned, not built).** Adding an i18n framework (e.g. react-i18next) and extracting the hardcoded English JSX strings; a parallel CJK typographic treatment (a Noto Sans JP/TC font stack, and dropping the `lowercase-title` transform — lowercasing is meaningless in Japanese/Chinese and the serif display stack has no CJK glyphs); an explicit `ui_language` preference on `users_meta` that `displayLocale()` reads (replacing the browser-language default); localizing category *display* labels while keeping the **stored category enum in English** (it is the join key, the glyph key, and the contract-test key — §6.2, only the rendered label is translated); and localizing the digest email template and its Sonnet narrative together. These are tracked in `TODO.md`.
